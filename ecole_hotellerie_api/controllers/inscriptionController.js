...
const updateInscriptionById = (req, res) => {
    const { id } = req.params;
    const { ID_Etudiant, DateDebutInscription, DateFinInscription, FraisInscription, FraisTransport, MontantInscription } = req.body;
    const sql = 'UPDATE inscriptions SET ID_Etudiant = ?, DateDebutInscription = ?, DateFinInscription = ?, FraisInscription = ?, FraisTransport = ?, MontantInscription = ? WHERE ID_Inscription = ?';
    connection.query(sql, [ID_Etudiant, DateDebutInscription, DateFinInscription, FraisInscription, FraisTransport, MontantInscription, id], (err, result) => {
        if (err) return res.status(500).send(err.toString());
