...
const createAbsence = (req, res) => {
    const { ID_Absence, ID_Etudiant,  DateDebutAbsence, DateFinAbsence ,created_at,updated_at} = req.body;
    const sql = 'INSERT INTO absence (ID_Absence, ID_Etudiant, DateDebutAbsence, DateFinAbsence,created_at,updated_at) VALUES (?,?, ?, ?, ?, ?)';
    connection.query(sql, [ID_Absence, ID_Etudiant, DateDebutAbsence, DateFinAbsence,created_at,updated_at], (err, result) => {
        if (err) return res.status(500).send(err.toString());
...
const updateAbsenceById = (req, res) => {
    const { id } = req.params;
    const { ID_Absence, ID_Etudiant,  DateDebutAbsence, DateFinAbsence,created_at,updated_at } = req.body;
    const sql = 'UPDATE absence SET ID_Absence = ?, ID_Etudiant = ?,  DateDebutAbsence = ?, DateFinAbsence = ?, created_at=?,updated_at=NOW() WHERE ID_Absence = ?';
    connection.query(sql, [ID_Absence, ID_Etudiant,  DateDebutAbsence, DateFinAbsence,created_at,updated_at, id], (err, result) => {
        if (err) return res.status(500).send(err.toString());
